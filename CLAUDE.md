...
## Roadmap proposée
1. ✅ **Débloquer le signup (1 session)** — FAIT le 2026-07-12, commit `c268f31`. Cause racine : schéma Drizzle non conforme à better-auth v1.5 (PK `id` manquante sur `account`, timestamps manquants sur `session`, table `verification` absente) — pas `$defaultFn`. Fix + test d'intégration PGlite (84/84 verts). Reste post-deploy : `GET /api/apply-migration` sur le domaine prod puis inscription avec un email neuf (cf. `.continue-here.md`).
