...
must_haves:
  truths:
    - "Database connection works with Neon serverless"
    - "All schema tables are defined with correct types"
    - "Migrations can be generated"
  artifacts:
    - path: "src/db/schema.ts"
      provides: "Complete database schema with all tables and enums"
      min_lines: 150
      contains: "pgTable"
    - path: "src/db/index.ts"
      provides: "Neon serverless connection"
      contains: "drizzle"
    - path: "drizzle.config.ts"
      provides: "Drizzle configuration for PostgreSQL"
      contains: "postgresql"
  key_links:
    ...
    - from: "drizzle.config.ts"
      to: "src/db/schema.ts"
      via: "schema path"
      pattern: "schema.*src/db"
---

<objective>
Set up Drizzle ORM with Neon PostgreSQL serverless, defining the complete database schema.

Purpose: Establish type-safe database layer with all tables needed for the MVP.
Output: Complete schema with users, groups, matches, match_players, ratings, player_stats tables.
</objective>
...
<task type="auto">
  <name>Task 1: Install Drizzle ORM and Neon driver</name>
  <files>package.json</files>
  <action>
Install Drizzle ORM with PostgreSQL support and Neon serverless driver:
```bash
...
```
  ...
  <verify>
    <automated>grep -q "drizzle-orm" package.json &amp;&amp; grep -q "db:generate" package.json</automated>
  </verify>
  <done>Drizzle ORM and Neon driver installed, database scripts added to package.json.</done>
</task>

<task type="auto">
  <name>Task 2: Create Drizzle config and Neon connection</name>
...
Create src/db/index.ts with Neon serverless connection:
```typescript
...
```
  ...
  <verify>
    <automated>test -f drizzle.config.ts &amp;&amp; test -f src/db/index.ts</automated>
  </verify>
  <done>Drizzle config and Neon connection established.</done>
</task>

<task type="auto">
  <name>Task 3: Define complete database schema</name>
  <files>src/db/schema.ts</files>
  <action>
Create src/db/schema.ts with ALL tables from DESIGN_DOC.md section 4.2.

IMPORTANT: Per CONTEXT.md D-15, use `captain | manager | player` for group_role enum (not `organizer | player`).
...
// Per CONTEXT.md: captain | manager | player (not organizer | player)
...
```
  ...
  <verify>
    <automated>pnpm typecheck &amp;&amp; grep -q "captain" src/db/schema.ts &amp;&amp; grep -q "playerStats" src/db/schema.ts</automated>
  </verify>
  <done>Complete database schema with all 6 tables, enums, relations, and type exports. Uses captain/manager/player roles per CONTEXT.md.</done>
</task>
...
- Schema file contains all tables from DESIGN_DOC.md
- group_role enum uses captain | manager | player (per CONTEXT.md)
- All relations are defined
- Type exports are available
</verification>

<success_criteria>
- Drizzle ORM and Neon serverless driver installed
- drizzle.config.ts configured for PostgreSQL
- src/db/index.ts exports db connection
- src/db/schema.ts contains complete schema (users, groups, group_members, matches, match_players, ratings, player_stats)
- All enums defined (match_status, player_status, team, recurrence, group_role)
